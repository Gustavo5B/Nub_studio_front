  ...
  return (
    <>
      <style>{`
        ...
        .as-header { padding: 24px 20px 16px; flex-shrink: 0; }
        .as-logo { height: 30px; margin-bottom: 20px; cursor: pointer; }

        .as-chip {
          background: #F9F8FC; border: 1px solid ${C.border};
        ...
        .as-name { font-size: 13px; font-weight: 800; color: ${C.text}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .as-status { font-size: 11px; color: ${C.muted}; margin-top: 2px; display: flex; align-items: center; gap: 4px; }
        .as-dot { width: 6px; height: 6px; border-radius: 50%; background: ${C.green}; flex-shrink: 0; }

        .as-nav { flex: 1; overflow-y: auto; padding: 8px 14px; min-height: 0; }
        .as-nav::-webkit-scrollbar { width: 3px; }
        .as-nav::-webkit-scrollbar-thumb { background: ${C.border}; border-radius: 4px; }
        .as-section-label { font-size: 10px; font-weight: 800; color: ${C.muted}; text-transform: uppercase; letter-spacing: 1.5px; padding: 0 8px; margin-bottom: 8px; }

        .as-nav-btn {
          display: flex; align-items: center; gap: 11px;
          width: 100%; padding: 11px 13px; border-radius: 11px; margin-bottom: 3px;
          background: transparent; border: 1px solid transparent;
          color: ${C.muted}; font-size: 13px; font-weight: 500;
          cursor: pointer; font-family: 'Outfit', sans-serif;
          transition: all .18s; text-align: left;
        }
        .as-nav-btn:hover { background: #F3F2F8; color: ${C.text}; }
        .as-nav-btn.active {
          background: ${C.orange}10; border-color: ${C.orange}30;
          color: ${C.orange}; font-weight: 700;
        }
        .as-chevron { margin-left: auto; }

        .as-divider { height: 1px; background: ${C.border}; margin: 12px 0; }
        ...
        .as-new-btn:hover { opacity: .88; transform: translateY(-1px); }

        .as-footer { padding: 12px 14px; border-top: 1px solid ${C.border}; flex-shrink: 0; }
        .as-logout {
          display: flex; align-items: center; gap: 10px;
        ...
        .as-logout:hover { color: ${C.pink}; background: ${C.pink}10; border-color: ${C.pink}22; }
        .as-logout-label { flex: 1; text-align: left; }

        /* Mobile compact */
        @media (max-width: 900px) {
          .as-sidebar { width: 240px; }
          .as-header { padding: 18px 16px 14px; }
          .as-logo { height: 26px; margin-bottom: 16px; }
          .as-nav { padding: 6px 12px; }
          .as-nav-btn { padding: 10px 12px; font-size: 13px; }
          .as-footer { padding: 10px 12px; }
        }
      `}</style>
