...
import {
  Database, Download, CheckCircle,
  Clock, RefreshCw, Shield, HardDrive,
...
const C = {
  ...
  creamSub:"#D8CABC", creamMut:"rgba(255,232,200,0.35)",
...
function BackupRow({ entry, onDelete, deleting }: { entry:BackupEntry; onDelete:(id:string)=>void; deleting:boolean }) {
  ...
  return (
    <div
      ...
      style={{ display:"grid", gridTemplateColumns:"1fr 90px 80px 70px 60px 80px", alignItems:"center", gap:12, padding:"13px 18px", borderRadius:10, background: hovered ? "rgba(141,76,205,0.06)" : "transparent", border:`1px solid ${hovered ? "rgba(141,76,205,0.20)" : "transparent"}`, transition:"all .15s", opacity: deleting ? 0.5 : 1 }}>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:6 }}>
        <a href={entry.url_archivo!} download={entry.filename} title="Descargar desde Supabase Storage"
        ...
        <button onClick={() => onDelete(entry.id)} disabled={deleting} title="Eliminar de Storage y BD"
