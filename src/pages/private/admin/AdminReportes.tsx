...
import {
  ...
  Layers, Database, RefreshCw, Download,
...
const ChartTip = ({ active, payload, label }: TooltipProps) => {
  ...
  return (
    <div style={{ background: "rgba(10,7,20,0.98)", border: `1px solid ${C.borderBr}`, borderRadius: 10, padding: "10px 14px", fontFamily: FB, boxShadow: "0 8px 28px rgba(0,0,0,0.5)" }}>
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12, fontWeight: 600, color: C.creamSub, marginBottom: 3 }}>
    ...
    <div style={{ width: 220, minHeight: "100vh", background: C.bgDeep, borderRight: `1px solid ${C.borderBr}`, display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", flexShrink: 0, zIndex: 40 }}>
      ...
      <div style={{ padding: "20px 18px 16px", borderBottom: `1px solid ${C.borderBr}` }}>
        <div onClick={() => navigate("/")} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 16 }}>
          <img src={logoImg} alt="Galería" style={{ width: 34, height: 34, borderRadius: 9, objectFit: "contain", flexShrink: 0, background: C.bgDeep, border: `1px solid ${C.orange}30` }} />
          <div>
            <div style={{ fontSize: 16, fontWeight: 900, color: C.cream, lineHeight: 1.2, fontFamily: FD }}>Galería</div>
            <div style={{ fontSize: 9, color: C.orange, letterSpacing: "0.16em", textTransform: "uppercase", fontFamily: FB, fontWeight: 700 }}>Panel Admin</div>
          </div>
        </div>
      ...
      <div style={{ padding: "12px 10px 18px", borderTop: `1px solid ${C.borderBr}` }}>
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={onLogout}
            style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "9px", borderRadius: 9, border: `1px solid rgba(204,89,173,0.25)`, background: "rgba(204,89,173,0.06)", cursor: "pointer", fontSize: 12, color: C.pink, fontWeight: 600, fontFamily: FB }}
...
// ── Topbar ────────────────────────────────────────────────────────────────────
function Topbar({ loading, onRefresh, navigate }: {
  loading: boolean; onRefresh: () => void; navigate: (p: string) => void;
}) {
  return (
...
// ── ExportBtn ─────────────────────────────────────────────────────────────────
function ExportBtn({ label, endpoint, color, icon: Icon }: {
  label: string; endpoint: string; color: string; icon: React.ElementType;
}) {
  ...
  const handleExport = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/reportes/${endpoint}`, {
      ...
      a.download  = match?.[1] || `reporte-${Date.now()}.xlsx`;
      ...
      showToast(`Error al exportar ${label}`, "err");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <Icon size={15} color={loading ? C.creamMut : color} strokeWidth={1.8}
          style={{ animation: loading ? "spin 1s linear infinite" : "none", flexShrink: 0 }} />
...
function ImportadorExcel({ tipo }: { tipo: ImportTipo }) {
  ...
  const procesar = async (file: File) => {
    ...
    try {
      ...
      const res  = await fetch(`${API_URL}/api/reportes/${cfg.importEndpoint}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${authService.getToken()}` },
        body: form,
      });
      ...
      setResultado(json);
      showToast(
        `Importación completa · ${json.resumen.insertadas} nuevos · ${json.resumen.actualizadas} actualizados`,
        "ok",
      );
    } catch (err: any) {
      showToast(err.message || "Error al importar", "err");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handlePlantilla = async () => {
    setPlantLoad(true);
    try {
      const res  = await fetch(`${API_URL}/api/reportes/${cfg.plantillaEndpoint}`, {
        headers: { Authorization: `Bearer ${authService.getToken()}` },
      });
      ...
      showToast("Error al descargar plantilla", "err");
    } finally {
      setPlantLoad(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        ...
        <div style={{ padding: "16px", borderRadius: 12, background: "rgba(255,232,200,0.03)", border: `1px solid ${C.borderBr}`, display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.55, margin: 0 }}>
            {cfg.nota}
          </p>
          ...
          >
            <Download size={13} strokeWidth={1.8}
              style={{ animation: plantLoad ? "spin 1s linear infinite" : "none" }} />
        ...
        <div style={{ padding: "16px", borderRadius: 12, background: "rgba(255,232,200,0.03)", border: `1px solid ${C.borderBr}`, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.55, margin: 0 }}>
            Campos disponibles en la hoja principal:
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {cfg.campos.map(c => (
              <span key={c} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 100, background: `${cfg.color}0D`, border: `1px solid ${cfg.color}20`, color: C.creamSub, fontFamily: FB }}>
                {c}
              </span>
            ))}
          </div>
        ...
        <div style={{ padding: "16px", borderRadius: 12, background: "rgba(255,232,200,0.03)", border: `1px solid ${C.borderBr}`, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.55, margin: "0 0 auto" }}>
            Arrastra el archivo al área de abajo o haz clic para seleccionarlo. Recibirás un reporte fila por fila con el resultado de cada operación.
          </p>
          <div style={{ display: "flex", gap: 6 }}>
            {[
              { label: "Nueva", color: C.green  },
              { label: "Editada", color: C.orange },
              { label: "Error", color: C.pink   },
            ].map(({ label, color }) => (
      ...
      <div
        ...
        style={{
          border: `2px dashed ${dragging ? cfg.color : uploading ? C.purple : C.borderBr}`,
          borderRadius: 12, padding: "32px 20px", textAlign: "center",
          cursor: uploading ? "wait" : "pointer",
          background: dragging ? `${cfg.color}06` : "transparent",
          transition: "all .2s",
          marginBottom: resultado ? 16 : 0,
        }}
      >
        ...
        <div style={{ marginTop: 4 }}>
          ...
          <div style={{ borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            <div style={{ maxHeight: 280, overflowY: "auto" }}>
              {resultado.detalle.map((r, i) => (
                <div key={i} style={{
                  display: "grid", gridTemplateColumns: "28px 46px 1fr 80px 80px", gap: 10,
                  alignItems: "center", padding: "9px 14px",
                  borderBottom: i < resultado.detalle.length - 1 ? `1px solid rgba(255,232,200,0.04)` : "none",
                  background: r.accion === "error" ? `${C.pink}06` : "transparent",
                }}>
                  {/* Icono */}
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    {r.accion === "error" && (
                      <span style={{ fontSize: 10, color: C.pink, fontFamily: FB, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "block", marginTop: 1 }}>
                        {r.errores.join(" · ")}
                      </span>
                    )}
                  </div>
          ...
          <div style={{ textAlign: "right", marginTop: 10 }}>
            <button onClick={() => setResultado(null)}
              style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, background: "transparent", border: "none", cursor: "pointer", textDecoration: "underline" }}>
              Limpiar resultado
            </button>
          </div>
        </div>
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px 0" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, marginTop: 3 }}>
            Exporta reportes en Excel o importa registros masivamente
          </div>
        </div>
      ...
      <div style={{ padding: "22px 24px 26px" }}>
        ...
        {mainTab === "exportar" && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.creamMut, fontFamily: FB, textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 14 }}>
              Reportes disponibles · formato .xlsx
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>

              {/* Ventas */}
              <div style={{ padding: "16px", borderRadius: 12, background: "rgba(255,132,14,0.04)", border: `1px solid rgba(255,132,14,0.12)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: `${C.orange}14`, border: `1px solid ${C.orange}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <FileText size={13} color={C.orange} strokeWidth={2} />
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.cream, fontFamily: FB }}>Ventas</span>
                </div>
                <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.5, margin: "0 0 12px" }}>
                  Fecha · obra · artista · cliente · totales · estado · método de pago
                </p>
                <ExportBtn label="Exportar ventas"    endpoint="exportar/ventas"    color={C.orange} icon={FileText} />
              </div>

              {/* Financiero */}
              <div style={{ padding: "16px", borderRadius: 12, background: "rgba(141,76,205,0.04)", border: `1px solid rgba(141,76,205,0.12)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: `${C.purple}14`, border: `1px solid ${C.purple}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Activity size={13} color={C.purple} strokeWidth={2} />
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.cream, fontFamily: FB }}>Financiero</span>
                </div>
                <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.5, margin: "0 0 12px" }}>
                  Ingresos brutos · comisiones de plataforma · neto artistas por mes
                </p>
                <ExportBtn label="Exportar financiero" endpoint="exportar/financiero" color={C.purple} icon={Activity} />
              </div>

              {/* Artistas */}
              <div style={{ padding: "16px", borderRadius: 12, background: "rgba(204,89,173,0.04)", border: `1px solid rgba(204,89,173,0.12)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: `${C.pink}14`, border: `1px solid ${C.pink}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Users size={13} color={C.pink} strokeWidth={2} />
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.cream, fontFamily: FB }}>Artistas</span>
                </div>
                <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.5, margin: "0 0 12px" }}>
                  Nombre · obras activas · ventas · comisiones pagadas y pendientes
                </p>
                <ExportBtn label="Exportar artistas"  endpoint="exportar/artistas"  color={C.pink}   icon={Users} />
              </div>

              {/* Catálogo */}
              <div style={{ padding: "16px", borderRadius: 12, background: "rgba(121,170,245,0.04)", border: `1px solid rgba(121,170,245,0.12)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: `${C.blue}14`, border: `1px solid ${C.blue}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Layers size={13} color={C.blue} strokeWidth={2} />
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.cream, fontFamily: FB }}>Catálogo obras</span>
                </div>
                <p style={{ fontSize: 11, color: C.creamMut, fontFamily: FB, lineHeight: 1.5, margin: "0 0 12px" }}>
                  Título · artista · precio · categoría · técnica · estado de cada obra
                </p>
                <ExportBtn label="Exportar catálogo"  endpoint="exportar/catalogo-obras" color={C.blue} icon={Layers} />
              </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22 }}>
              ...
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 100, background: `${IMPORTADOR_CONFIG[importTab].color}0D`, border: `1px solid ${IMPORTADOR_CONFIG[importTab].color}25` }}>
                ...
                <span style={{ fontSize: 11, color: IMPORTADOR_CONFIG[importTab].color, fontWeight: 700, fontFamily: FB }}>
                  Importar {importTab}
                </span>
              </div>
            </div>
            ...
            {/* Componente importador genérico — key fuerza reset al cambiar tipo */}
        ...
        <main style={{ flex: 1, padding: "24px 28px 36px", overflowY: "auto" }}>
          ...
          <div style={{ marginBottom: 22, display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
            <div>
              ...
              <p style={{ margin: "4px 0 0", fontSize: 13, color: C.creamMut, fontFamily: FB }}>
                {new Date().getFullYear()} · datos en tiempo real
              </p>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px 18px 12px", overflow: "hidden" }}>
                  ...
                  <AreaChart data={ivsMes} margin={{ top: 4, right: 4, bottom: 4, left: -14 }}>
                    <defs>
                      <linearGradient id="gIngresos" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={C.orange} stopOpacity={0.25} /><stop offset="100%" stopColor={C.orange} stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="gNeto" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={C.purple} stopOpacity={0.20} /><stop offset="100%" stopColor={C.purple} stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="gCom" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={C.pink} stopOpacity={0.15} /><stop offset="100%" stopColor={C.pink} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid stroke="rgba(255,232,200,0.05)" strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="mes" stroke="transparent" tick={{ fill: C.creamMut, fontSize: 10, fontFamily: FB }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px" }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: "center", padding: "40px 0", color: C.creamMut, fontSize: 13, fontFamily: FB }}>
                  <Trophy size={24} color={C.creamMut} style={{ marginBottom: 8, opacity: .2 }} />
                  <div>Sin ventas aún</div>
                </div>
              ) : topObras.map((obra, i) => {
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px" }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: "center", padding: "40px 0", color: C.creamMut, fontSize: 13, fontFamily: FB }}>
                  <Users size={24} color={C.creamMut} style={{ marginBottom: 8, opacity: .2 }} />
                  <div>Sin comisiones aún</div>
                </div>
              ) : topArtistas.map((a, i) => (
                <div key={a.id_artista}
                  ...
                  <div style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0, background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "white" }}>
                    {a.foto_perfil
                      ? <img src={a.foto_perfil} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      : (a.nombre_artistico || a.nombre_completo)?.[0]?.toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10.5, color: C.creamMut, fontFamily: FB, marginTop: 1 }}>
                      {a.ventas_totales} {a.ventas_totales === 1 ? "venta" : "ventas"}
                    </div>
                  </div>
