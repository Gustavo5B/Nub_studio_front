...
const ESTADOS: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  agotada:   { label: "Agotada",   color: C.creamMut,icon: Package     },
};
...
// MODAL ELIMINAR
// ─────────────────────────────────────────────────────────────
function ModalEliminar({ obra, onConfirm, onCancel }: { obra:Obra; onConfirm:()=>void; onCancel:()=>void }) {
  return (
      ...
      <div style={{ background:"rgba(16,13,28,0.98)", border:`1px solid ${C.borderBr}`, borderRadius:22, padding:"34px", maxWidth:420, width:"90%", boxShadow:"0 40px 80px rgba(0,0,0,0.7)", backdropFilter:"blur(40px)", animation:"modalIn .25s cubic-bezier(0.16,1,0.3,1)", fontFamily:FB }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"12px", borderRadius:11, border:`1px solid ${C.border}`, background:"rgba(255,232,200,0.04)", color:C.creamSub, fontWeight:600, fontSize:13.5, cursor:"pointer", fontFamily:FB, transition:"all .15s" }}
            ...
            onMouseLeave={e=>{ (e.currentTarget as HTMLElement).style.borderColor=C.border;   (e.currentTarget as HTMLElement).style.color=C.creamSub; }}
          >Cancelar</button>
          <button onClick={onConfirm} style={{ flex:1, padding:"12px", borderRadius:11, border:"none", background:`linear-gradient(135deg,${C.pink},${C.purple})`, color:"white", fontWeight:700, fontSize:13.5, cursor:"pointer", fontFamily:FB, boxShadow:`0 6px 22px ${C.pink}38`, transition:"transform .15s, box-shadow .15s" }}
...
// ─────────────────────────────────────────────────────────────
function ModalEstado({ obra, onConfirm, onCancel }: { obra:Obra; onConfirm:(estado:string)=>void; onCancel:()=>void }) {
  const [selected, setSelected] = useState(obra?.estado || "pendiente");
  return (
    <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(8,5,18,0.80)", backdropFilter:"blur(10px)" }}>
      <div style={{ background:"rgba(16,13,28,0.98)", border:`1px solid ${C.borderBr}`, borderRadius:22, padding:"32px", maxWidth:400, width:"90%", boxShadow:"0 40px 80px rgba(0,0,0,0.7)", backdropFilter:"blur(40px)", animation:"modalIn .25s cubic-bezier(0.16,1,0.3,1)", fontFamily:FB }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          <div style={{ fontSize:19, fontWeight:900, color:C.cream, fontFamily:FD }}>Cambiar estado</div>
          <button onClick={onCancel} style={{ width:32, height:32, borderRadius:9, background:"rgba(255,232,200,0.05)", border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"border-color .15s" }}
            onMouseEnter={e=>(e.currentTarget as HTMLElement).style.borderColor=C.borderHi}
            onMouseLeave={e=>(e.currentTarget as HTMLElement).style.borderColor=C.border}
          ><X size={15} color={C.creamMut} /></button>
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:26 }}>
          {Object.entries(ESTADOS).map(([key, { label, color, icon:Icon }]) => {
            ...
            return (
              <button key={key} onClick={()=>setSelected(key)} style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 15px", borderRadius:13, border:`1.5px solid ${on?`${color}55`:C.border}`, background:on?`${color}14`:"rgba(255,232,200,0.02)", cursor:"pointer", textAlign:"left", fontFamily:FB, transition:"all .15s" }}
              ...
              >
                <div style={{ width:34, height:34, borderRadius:9, background:`${color}18`, border:`1px solid ${color}30`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:on?`0 2px 10px ${color}30`:"none" }}>
                  <Icon size={15} color={color} strokeWidth={2} />
                </div>
                <span style={{ fontSize:14, fontWeight:on?700:400, color:on?C.cream:C.creamSub, fontFamily:FB }}>{label}</span>
                {on && <div style={{ marginLeft:"auto", width:8, height:8, borderRadius:"50%", background:color, boxShadow:`0 0 9px ${color}` }} />}
              </button>
            );
          })}
        </div>
        <button onClick={()=>onConfirm(selected)} style={{ width:"100%", padding:"13px", borderRadius:11, border:"none", background:`linear-gradient(135deg,${C.orange},${C.magenta})`, color:"white", fontWeight:700, fontSize:14.5, cursor:"pointer", fontFamily:FB, boxShadow:`0 6px 22px ${C.orange}40`, transition:"transform .15s, box-shadow .15s" }}
...
export default function ListaObras() {
  ...
  const [modalEliminar,   setModalEliminar]   = useState<Obra|null>(null);
  const [modalEstado,     setModalEstado]     = useState<Obra|null>(null);
  ...
  }, [page, filtroEstado, filtroCategoria, search, showToast]);
  ...
  const handleEliminar = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/obras/${modalEliminar.id_obra}`, {
        method: "DELETE", headers: { Authorization: `Bearer ${authService.getToken()}` },
      });
      if (!res.ok) { showToast(await handleApiError(res), "err"); return; }
      showToast("Obra eliminada correctamente", "ok");
  ...
  const handleCambiarEstado = async (nuevoEstado: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/obras/${modalEstado.id_obra}`, {
        method: "PUT",
      ...
      showToast(`Estado cambiado a "${label}"`, "ok");
      setModalEstado(null);
      cargarObras();
    } catch(err) {
      showToast(handleNetworkError(err), "err");
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, position:"relative", zIndex:1 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 32px", height:64, background:"rgba(10,7,20,0.88)", borderBottom:`1px solid ${C.borderBr}`, backdropFilter:"blur(24px)", WebkitBackdropFilter:"blur(24px)", position:"sticky", top:0, zIndex:30, fontFamily:FB }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"5px 13px", borderRadius:100, background:`linear-gradient(135deg,${C.blue}20,${C.purple}10)`, border:`1px solid ${C.blue}38`, fontSize:11, fontWeight:700, color:C.blue, letterSpacing:"0.09em", textTransform:"uppercase", fontFamily:FB }}>
            ...
            <span style={{ fontSize:12, padding:"3px 10px", borderRadius:100, background:`${C.blue}15`, border:`1px solid ${C.blue}30`, color:C.blue, fontWeight:700, fontFamily:FB }}>{total} obras</span>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <button onClick={cargarObras} style={{ width:38, height:38, borderRadius:10, background:"rgba(255,232,200,0.04)", border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"all .15s" }}
            ...
            <button onClick={()=>navigate("/admin/obras/crear")} style={{ display:"flex", alignItems:"center", gap:7, background:`linear-gradient(135deg,${C.orange},${C.magenta})`, border:"none", color:"white", padding:"9px 18px", borderRadius:10, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:FB, boxShadow:`0 6px 22px ${C.orange}40`, transition:"transform .15s, box-shadow .15s" }}
              onMouseEnter={e=>{ (e.currentTarget as HTMLElement).style.transform="translateY(-1px)"; (e.currentTarget as HTMLElement).style.boxShadow=`0 10px 30px ${C.orange}55`; }}
              onMouseLeave={e=>{ (e.currentTarget as HTMLElement).style.transform="translateY(0)";    (e.currentTarget as HTMLElement).style.boxShadow=`0 6px 22px ${C.orange}40`; }}
            ><Plus size={15} strokeWidth={2.5} /> Nueva Obra</button>
          </div>
        </div>
        ...
        <main style={{ flex:1, padding:"26px 32px", overflowY:"auto" }}>
          ...
          <div style={{ marginBottom:22, animation:"fadeUp .45s ease both" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
              ...
              <span style={{ fontSize:11, fontWeight:700, color:C.creamMut, textTransform:"uppercase", letterSpacing:"0.12em", fontFamily:FB }}>Catálogo de arte</span>
            </div>
            <h1 style={{ fontSize:26, fontWeight:900, margin:0, fontFamily:FD, color:C.cream, letterSpacing:"-0.02em" }}>
              Gestión de{" "}
              <span style={{ background:`linear-gradient(90deg,${C.blue},${C.purple})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Obras</span>
            </h1>
          </div>

          {/* Filtros */}
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px", marginBottom:18, display:"flex", gap:14, flexWrap:"wrap", alignItems:"center", animation:"fadeUp .5s ease .05s both" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, flex:1, minWidth:220, background:"rgba(255,232,200,0.04)", border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 14px" }}>
              ...
              <input value={search} onChange={e=>{ setSearch(e.target.value); setPage(1); }} placeholder="Buscar por título o artista…"
            ...
            <select value={filtroCategoria} onChange={e=>{ setFiltroCategoria(e.target.value); setPage(1); }}
              style={{ padding:"10px 14px", borderRadius:10, border:`1px solid ${C.border}`, background:"rgba(255,232,200,0.04)", fontSize:13, color:C.creamSub, cursor:"pointer", fontFamily:FB, outline:"none" }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {FILTROS.map(({ key, label, color }) => {
                ...
                return (
                  <button key={key} onClick={()=>{ setFiltroEstado(key); setPage(1); }} style={{ padding:"8px 16px", borderRadius:100, border:`1.5px solid ${on?`${color}55`:C.border}`, background:on?`${color}15`:"rgba(255,232,200,0.03)", color:on?color:C.creamSub, fontWeight:on?700:500, fontSize:12.5, cursor:"pointer", fontFamily:FB, transition:"all .15s", boxShadow:on?`0 2px 12px ${color}25`:"none" }}
          ...
          {/* Tabla */}
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, overflow:"hidden", backdropFilter:"blur(20px)", animation:"fadeUp .5s ease .1s both" }}>
            {loading ? (
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:240, color:C.creamMut, gap:12, fontSize:13.5, fontFamily:FB }}>
                <RefreshCw size={18} strokeWidth={1.8} style={{ animation:"spin 1s linear infinite", color:C.orange }} /> Cargando obras…
              </div>
            ) : obras.length === 0 ? (
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:240, color:C.creamMut, gap:12 }}>
                ...
                <button onClick={()=>navigate("/admin/obras/crear")} style={{ fontSize:13, color:C.orange, background:`${C.orange}12`, border:`1px solid ${C.orange}35`, borderRadius:8, padding:"7px 16px", cursor:"pointer", fontWeight:700, fontFamily:FB }}>
                  + Crear primera obra
                </button>
              </div>
            ) : (
                      ...
                      >
                        ...
                        <td style={{ padding:"14px 16px", borderRight:`1px solid ${C.border}` }}>
                          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                            <div style={{ width:44, height:44, borderRadius:11, background:`${C.blue}15`, border:`1px solid ${C.blue}30`, overflow:"hidden", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 2px 10px ${C.blue}18` }}>
                            ...
                            <div>
                              <div style={{ fontSize:13.5, fontWeight:700, color:C.cream, maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontFamily:FB }}>{obra.titulo}</div>
                        ...
                        <td style={{ padding:"14px 16px", borderRight:`1px solid ${C.border}` }}>
                          <button onClick={()=>setModalEstado(obra)} title="Click para cambiar estado"
                            style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 13px", borderRadius:20, background:`${estado.color}15`, border:`1px solid ${estado.color}35`, color:estado.color, fontSize:12, fontWeight:800, cursor:"pointer", fontFamily:FB, transition:"all .15s", boxShadow:estado.color===C.green?`0 0 10px ${C.green}25`:"none" }}
                        ...
                        <td style={{ padding:"14px 16px", borderRight:`1px solid ${C.border}` }}>
                          <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13.5, color:C.creamSub, fontFamily:FB }}>
                            <Eye size={13} color={C.purple} strokeWidth={1.8} /> {obra.vistas || 0}
                          </div>
                        </td>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:18, animation:"fadeUp .5s ease .15s both" }}>
              ...
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                  style={{ width:36, height:36, borderRadius:9, border:`1px solid ${C.border}`, background:"rgba(255,232,200,0.03)", display:"flex", alignItems:"center", justifyContent:"center", cursor:page===1?"not-allowed":"pointer", opacity:page===1?0.35:1, transition:"all .15s" }}>
                ...
                {Array.from({ length:Math.min(5,totalPages) },(_,i)=>{
                  ...
                  return (
                    <button key={p} onClick={()=>setPage(p)} style={{ width:36, height:36, borderRadius:9, border:`1px solid ${isActive?`${C.orange}60`:C.border}`, background:isActive?`linear-gradient(135deg,${C.orange},${C.magenta})`:"rgba(255,232,200,0.03)", color:isActive?"white":C.creamSub, fontWeight:isActive?800:500, fontSize:13.5, cursor:"pointer", fontFamily:FB, boxShadow:isActive?`0 4px 16px ${C.orange}40`:"none", transition:"all .15s" }}>{p}</button>
                  );
                })}
                <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
                  style={{ width:36, height:36, borderRadius:9, border:`1px solid ${C.border}`, background:"rgba(255,232,200,0.03)", display:"flex", alignItems:"center", justifyContent:"center", cursor:page===totalPages?"not-allowed":"pointer", opacity:page===totalPages?0.35:1, transition:"all .15s" }}>
