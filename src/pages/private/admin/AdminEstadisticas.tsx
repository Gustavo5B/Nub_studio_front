...
function KpiCard({ label, value, contexto, sub, accent, icon:Icon, tendencia, animating }: {
  ...
  return (
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 18px", position:"relative", overflow:"hidden",
      ...
      boxShadow: animating ? `0 0 0 2px ${accent}40` : "none" }}
      onMouseEnter={e => {
        const el = e.currentTarget as HTMLElement;
        el.style.borderColor = `${accent}50`;
        el.style.transform = "translateY(-3px)";
        el.style.boxShadow = `0 12px 32px ${accent}20, 0 0 0 1px ${accent}25`;
      ...
      <div style={{ position:"absolute", inset:0, background:`radial-gradient(ellipse at 10% 0%, ${accent}10 0%, transparent 60%)`, pointerEvents:"none" }} />
      {/* Línea superior */}
      <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${accent},${accent}60,transparent)` }} />

      <div style={{ position:"relative" }}>
        <div style={{ width:34, height:34, borderRadius:10, background:`${accent}18`, border:`1px solid ${accent}30`, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:12,
          boxShadow:`0 4px 12px ${accent}20` }}>
          <Icon size={16} color={accent} strokeWidth={2} />
        </div>
        <div style={{ fontSize:28, fontWeight:900, color:C.cream, fontFamily:FD, lineHeight:1, marginBottom:5,
          transition:"color .3s", animation: animating ? "pulseNum .4s ease" : "none" }}>
          {value}
        </div>
        <div style={{ fontSize:12, fontWeight:700, color:C.creamSub, fontFamily:FB }}>{label}</div>
        <div style={{ fontSize:10.5, color:C.creamMut, fontFamily:FB, marginTop:2 }}>{contexto}</div>
        {sub && <div style={{ fontSize:10.5, color:accent, fontFamily:FB, marginTop:4, fontWeight:600 }}>{sub}</div>}
        {tendencia !== undefined && (
          <div style={{ marginTop:8, display:"inline-flex", alignItems:"center", gap:4, padding:"3px 9px", borderRadius:20,
            background: tendencia >= 0 ? "rgba(34,201,122,0.14)" : "rgba(240,78,107,0.14)",
            border:`1px solid ${tendencia >= 0 ? "rgba(34,201,122,0.30)" : "rgba(240,78,107,0.30)"}` }}>
            {tendencia >= 0 ? <TrendingUp size={11} color={C.green}/> : <TrendingDown size={11} color={C.red}/>}
            <span style={{ fontSize:11, fontWeight:800, color:tendencia>=0?C.green:C.red }}>
              {fmtP(tendencia)} vs ayer
            </span>
          </div>
        )}
      </div>
    ...
    <>
      ...
      <main style={{ flex:1, padding:"22px 26px 40px", overflowY:"auto", backgroundColor:C.bg, backgroundImage:`radial-gradient(ellipse at 85% 0%,rgba(255,132,14,0.07) 0%,transparent 45%),radial-gradient(ellipse at 5% 85%,rgba(141,76,205,0.07) 0%,transparent 40%)`, fontFamily:FB }}>
        ...
        <div style={{ marginBottom:22 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"4px 12px", borderRadius:100, background:"rgba(255,132,14,0.09)", border:`1px solid rgba(255,132,14,0.22)`, fontSize:11, color:C.orange, marginBottom:10, fontWeight:700, letterSpacing:"0.06em" }}>
            <Activity size={10} color={C.orange} /> ANÁLISIS · MODELO PREDICTIVO
          </div>
          <h1 style={{ fontSize:26, fontWeight:900, color:C.cream, fontFamily:FD, margin:"0 0 4px" }}>
            Estadísticas de <span style={{ color:C.orange, fontStyle:"italic" }}>Interacción</span>
          </h1>
          <p style={{ fontSize:13, color:C.creamMut, margin:0 }}>
            📅 Histórico desde <strong style={{ color:C.creamSub }}>{fechaInicio}</strong> hasta <strong style={{ color:C.creamSub }}>{fechaFin}</strong>
        ...
        {loading ? (
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:10, marginBottom:22, animation:"fadeUp .4s ease" }}>
        ...
        {/* ── TABS ── */}
        <div style={{ display:"flex", gap:3, marginBottom:20, background:"rgba(255,232,200,0.025)", padding:5, borderRadius:14, border:`1px solid ${C.border}`, flexWrap:"wrap" }}>
          {TABS.map(({ id, label, icon:Icon, desc }) => {
            const on = tab === id;
            return (
              <button key={id} onClick={() => changeTab(id)}
                style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2, padding:"8px 14px", borderRadius:10,
                  border: on ? `1px solid rgba(255,132,14,0.45)` : "1px solid transparent",
                  background: on ? "rgba(255,132,14,0.12)" : "transparent",
                  color: on ? C.orange : C.creamMut,
                  cursor:"pointer", fontFamily:FB, transition:"all .18s", whiteSpace:"nowrap", position:"relative" }}
                title={desc}
                onMouseEnter={e => { if(!on) { const el=e.currentTarget as HTMLElement; el.style.background="rgba(255,200,150,0.06)"; el.style.color=C.creamSub; } }}
                onMouseLeave={e => { if(!on) { const el=e.currentTarget as HTMLElement; el.style.background="transparent"; el.style.color=C.creamMut; } }}>
                {on && <div style={{ position:"absolute", bottom:0, left:"20%", right:"20%", height:2, borderRadius:99, background:C.orange }} />}
                <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                  <Icon size={12} strokeWidth={on?2.3:1.7} />
                  <span style={{ fontSize:12, fontWeight:on?700:500 }}>{label}</span>
                </div>
                <span style={{ fontSize:9, color:on?`${C.orange}90`:"rgba(255,232,200,0.18)", fontWeight:400 }}>{desc}</span>
              </button>
            );
          })}
        </div>
