...
function Sidebar({ navigate }: { navigate: (p: string) => void }) {
  ...
  return (
    <div style={{
      ...
      {/* Nav links — naranja único como activo, sin colores por item */}
...
// ── Card — accent line height:2, patrón unificado ─────────────────────────────
...
export default function CrearArtista() {
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === "number") setForm(p => ({ ...p, [name]: value === "" ? "" : Number(value) } as typeof form));
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 56, background: C.bgDeep, borderBottom: `1px solid ${C.borderBr}`, position: "sticky", top: 0, zIndex: 30, fontFamily: FB }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => navigate("/admin/artistas")} style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: "none", cursor: "pointer", color: C.creamMut, fontSize: 11.5, fontWeight: 700, fontFamily: FB, letterSpacing: "0.08em", textTransform: "uppercase", transition: "color .15s" }}
            ...
            <ChevronRight size={12} color={C.creamMut} />
            <span style={{ fontSize: 13, color: C.creamSub, fontFamily: FB }}>Nuevo</span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button form="form-crear-artista" type="submit" disabled={loading}
              ...
              {loading
                ...
                : <><Save size={14} strokeWidth={2.5} /> Crear Artista</>
              }
            </button>
          </div>
        </div>
                ...
                <Card accent={C.purple} icon={Tag} title="Categoría y matrícula" delay={0.12}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    ...
                    <div>
                      ...
                      {/* Badge de matrícula — borderRadius:100, patrón unificado */}
                ...
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", marginBottom: 14, position: "relative", animation: "fadeUp .5s ease .05s both" }}>
                  ...
                  <div style={{ padding: "0 18px 18px", marginTop: -28 }}>
                    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 12 }}>
                      ...
                      {est && (
                        <span style={{ padding: "3px 10px", borderRadius: 100, background: `${est.color}14`, border: `1px solid ${est.color}38`, color: est.color, fontSize: 10.5, fontWeight: 800, fontFamily: FB }}>
                          {est.label}
                        </span>
                      )}
                    </div>
