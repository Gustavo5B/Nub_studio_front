...
// ─────────────────────────────────────────────────────────────
// PALETA OFICIAL — idéntica a AdminDashboard + ListaObras
// ─────────────────────────────────────────────────────────────
const C = {
  orange:   "#FF840E",
  pink:     "#CC59AD",
            ...
            >
              ...
              {id === "artistas" && pendientes > 0 ? (
                <div style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 10.5, fontWeight: 900, color: "#000", padding: "0 6px",
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(8,5,18,0.82)", backdropFilter: "blur(10px)" }}>
      <div style={{
        ...
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          marginBottom: 20, boxShadow: `0 4px 18px ${C.pink}25`,
        }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={onConfirm} style={{
            ...
            boxShadow: `0 6px 22px ${C.pink}38`, transition: "transform .15s",
          }}
...
function ModalAprobacion({ artista, onConfirm, onCancel }: { artista: any; onConfirm: (accion: "activo" | "rechazado") => void; onCancel: () => void }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(8,5,18,0.82)", backdropFilter: "blur(10px)" }}>
      <div style={{
        background: "rgba(16,13,28,0.98)", border: `1px solid ${C.borderBr}`,
        borderRadius: 22, padding: "36px", maxWidth: 460, width: "90%",
        boxShadow: "0 40px 80px rgba(0,0,0,0.7)", backdropFilter: "blur(40px)",
        animation: "modalIn .25s cubic-bezier(0.16,1,0.3,1)", fontFamily: FB,
      }}>
        {/* Header */}
        ...
        <div style={{ fontSize: 14, color: C.creamSub, marginBottom: 22, lineHeight: 1.7, fontFamily: FB }}>
          ...
          {artista?.nombre_artistico && (
            <> (<span style={{ color: C.gold }}>✦ {artista.nombre_artistico}</span>)</>
          )}
        </div>
          ...
          {[
            ...
            <div key={label} style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 13.5, lineHeight: 1.6, alignItems: "flex-start" }}>
              <span style={{ color: C.creamMut, minWidth: 84, fontWeight: 600, fontFamily: FB, display: "flex", alignItems: "center", gap: 5 }}>
                {Icon && <Icon size={11} color={color!} strokeWidth={2} />}
                {label}:
              </span>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onCancel} style={{
            flex: 1, padding: "12px", borderRadius: 11, border: `1px solid ${C.border}`,
            background: "rgba(255,232,200,0.03)", color: C.creamSub,
            fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: FB, transition: "all .15s",
          }}
          ...
          <button onClick={() => onConfirm("rechazado")} style={{
            flex: 1, padding: "12px", borderRadius: 11,
            border: `1px solid ${C.pink}35`, background: `${C.pink}12`,
            color: C.pink, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: FB, transition: "all .15s",
          }}
          ...
          <button onClick={() => onConfirm("activo")} style={{
            flex: 1.3, padding: "12px", borderRadius: 11, border: "none",
            background: `linear-gradient(135deg, ${C.green}, #1aab67)`,
            color: "#000", fontWeight: 900, fontSize: 13, cursor: "pointer", fontFamily: FB,
            boxShadow: `0 6px 22px ${C.green}35`, transition: "transform .15s, box-shadow .15s",
          }}
...
export default function ListaArtistas() {
  ...
  const handleAprobacion = async (accion: "activo" | "rechazado") => {
    ...
    try {
      ...
      setModalAprob(null); cargarArtistas();
    } catch (err) { console.error(err); }
  };
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              ...
              background: `linear-gradient(135deg, ${C.pink}20, ${C.purple}10)`,
              border: `1px solid ${C.pink}38`,
            ...
            <span style={{ fontSize: 12, padding: "3px 10px", borderRadius: 100, background: `${C.pink}15`, border: `1px solid ${C.pink}30`, color: C.pink, fontWeight: 700, fontFamily: FB }}>
              {total} artistas
            </span>
            {pendientes > 0 && (
              <span style={{
                fontSize: 12, padding: "3px 10px", borderRadius: 100,
                background: `${C.gold}15`, border: `1px solid ${C.gold}38`,
                color: C.gold, fontWeight: 700, fontFamily: FB,
                boxShadow: `0 0 10px ${C.gold}25`,
              }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={cargarArtistas} style={{
            ...
            <button onClick={() => navigate("/admin/artistas/crear")} style={{
              display: "flex", alignItems: "center", gap: 7,
              background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`,
              border: "none", color: "white", padding: "9px 18px", borderRadius: 10,
              fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: FB,
              boxShadow: `0 6px 22px ${C.pink}40`, transition: "transform .15s, box-shadow .15s",
            }}
        ...
        <main style={{ flex: 1, padding: "26px 32px", overflowY: "auto" }}>
          ...
          {pendientes > 0 && filtroEstado === "todos" && (
            <div style={{
              ...
              marginBottom: 20,
              background: `linear-gradient(135deg, ${C.gold}12, ${C.orange}06)`,
              border: `1px solid ${C.gold}35`, borderRadius: 16,
              animation: "fadeUp .5s ease .05s both",
            }}>
              <div style={{
                width: 40, height: 40, borderRadius: 11,
                background: `${C.gold}22`, border: `1px solid ${C.gold}35`,
              ...
              <button onClick={() => setFiltroEstado("pendiente")} style={{
                padding: "8px 16px", borderRadius: 10,
                background: `${C.gold}22`, border: `1px solid ${C.gold}45`,
                color: C.gold, fontSize: 12.5, fontWeight: 700, cursor: "pointer", fontFamily: FB,
          ...
          <div style={{
            ...
            <div style={{
              ...
              background: "rgba(255,232,200,0.04)", border: `1px solid ${C.border}`,
              borderRadius: 10, padding: "10px 14px", transition: "border-color .15s",
            }}>
              ...
              {FILTROS.map(({ key, label, color }) => {
                ...
                return (
                  ...
                  >
                    {label}
                    {cnt !== undefined && cnt > 0 && (
                      <span style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.orange})`, color: "#000", borderRadius: 10, padding: "0 6px", fontSize: 10, fontWeight: 900 }}>
                        {cnt}
                      </span>
                    )}
                  </button>
          ...
          <div style={{
            ...
            {loading ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 240, color: C.creamMut, gap: 12, fontSize: 13.5, fontFamily: FB }}>
                <RefreshCw size={18} strokeWidth={1.8} style={{ animation: "spin 1s linear infinite", color: C.pink }} />
                Cargando artistas…
              </div>
            ) : artistas.length === 0 ? (
                  ...
                  {artistas.map((artista, i) => {
                    ...
                    return (
                      <tr key={artista.id_artista}
                        style={{
                          borderBottom: i < artistas.length - 1 ? `1px solid rgba(255,232,200,0.05)` : "none",
                          transition: "background .12s",
                          background: esPendiente ? `${C.gold}06` : "transparent",
                        }}
                      ...
                      >
                        ...
                        <td style={{ padding: "14px 16px", borderRight: `1px solid ${C.border}` }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <div style={{
                              ...
                              background: artista.foto_perfil ? "transparent" : `${avatarCol}18`,
                              overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center",
                              border: `1.5px solid ${esPendiente ? C.gold + "45" : avatarCol + "35"}`,
                              boxShadow: esPendiente ? `0 0 10px ${C.gold}25` : `0 2px 10px ${avatarCol}18`,
                            }}>
                              {artista.foto_perfil
                            ...
                            <div>
                              ...
                              {artista.matricula && (
                                <div style={{ fontSize: 10.5, color: C.creamMut, marginTop: 2, fontFamily: FB }}>{artista.matricula}</div>
                              )}
                            </div>
                          </div>
                        </td>
                        ...
                        <td style={{ padding: "14px 16px", borderRight: `1px solid ${C.border}` }}>
                          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                            {artista.correo && (
                              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: C.creamSub, fontFamily: FB }}>
                                <Mail size={11} strokeWidth={1.8} color={C.blue} />
                                <span style={{ maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{artista.correo}</span>
                              </div>
                            )}
                            {artista.telefono && (
                              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: C.creamSub, fontFamily: FB }}>
                                <Phone size={11} strokeWidth={1.8} color={C.purple} /> {artista.telefono}
                              </div>
                            )}
                          </div>
                        </td>
                        ...
                        {/* Comisión */}
                        <td style={{ padding: "14px 16px", borderRight: `1px solid ${C.border}` }}>
                          <span style={{ fontSize: 15, fontWeight: 900, color: C.gold, fontFamily: FD }}>
                        ...
                        <td style={{ padding: "14px 16px", borderRight: `1px solid ${C.border}` }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13.5, color: C.creamSub, fontFamily: FB }}>
                            <ImageIcon size={13} color={C.purple} strokeWidth={1.8} />
                        ...
                        <td style={{ padding: "14px 16px", borderRight: `1px solid ${C.border}` }}>
                          <button onClick={() => handleToggleEstado(artista)}
                            ...
                            style={{
                              ...
                              background: `${estado.color}15`, border: `1px solid ${estado.color}38`,
                              color: estado.color, fontSize: 12, fontWeight: 800,
                              cursor: "pointer", fontFamily: FB, transition: "all .15s",
                              boxShadow: esPendiente ? `0 0 12px ${C.gold}30` : estado.color === C.green ? `0 0 10px ${C.green}25` : "none",
                            }}
                            ...
                            {esPendiente && (
                              <button onClick={() => setModalAprob(artista)} title="Revisar solicitud"
                                style={{
                                  ...
                                  borderRadius: 8, background: `${C.gold}18`, border: `1px solid ${C.gold}38`,
                                  color: C.gold, fontSize: 11.5, fontWeight: 700, cursor: "pointer", fontFamily: FB,
                                  transition: "all .15s",
                                }}
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button key={p} onClick={() => setPage(p)} style={{
                      ...
                      border: `1px solid ${isActive ? `${C.pink}60` : C.border}`,
                      background: isActive ? `linear-gradient(135deg, ${C.pink}, ${C.purple})` : "rgba(255,232,200,0.03)",
                      color: isActive ? "white" : C.creamSub,
                      fontWeight: isActive ? 800 : 500, fontSize: 13.5,
                      cursor: "pointer", fontFamily: FB,
                      boxShadow: isActive ? `0 4px 16px ${C.pink}40` : "none",
                      transition: "all .15s",
                    }}>
                      {p}
                    </button>
