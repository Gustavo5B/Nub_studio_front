...
function WelcomeBanner({ userName }: { userName:string }) {
  ...
  return (
    <div style={{ borderRadius:14, padding:"22px 26px", background:`linear-gradient(135deg, rgba(255,132,14,0.08), rgba(141,76,205,0.05))`, border:`1px solid rgba(255,132,14,0.14)`, marginBottom:16, display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 10px", borderRadius:100, background:"rgba(255,248,238,0.05)", border:`1px solid ${C.borderBr}`, fontSize:11, color:C.creamMut, fontFamily:FB, marginBottom:10 }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:C.gold, display:"inline-block" }} /> {dateStr}
        </div>
...
function AlertaPendientes({ count, navigate }: { count:number; navigate:(p:string) => void }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 20px", borderRadius:12, background:`rgba(255,193,16,0.05)`, border:`1px solid rgba(255,193,16,0.28)`, marginBottom:16, fontFamily:FB, animation:"fadeUp .35s ease both" }}>
      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
        <div style={{ width:34, height:34, borderRadius:9, background:`${C.gold}12`, border:`1px solid ${C.gold}28`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11.5, color:C.creamMut, marginTop:2 }}>Revísalas para mantener el catálogo actualizado</div>
        </div>
      </div>
      <button onClick={() => navigate("/admin/obras?estado=pendiente")}
        style={{ display:"flex", alignItems:"center", gap:5, background:`${C.gold}12`, border:`1px solid ${C.gold}30`, color:C.gold, padding:"8px 16px", borderRadius:8, fontWeight:700, fontSize:12.5, cursor:"pointer", fontFamily:FB, transition:"all .15s", flexShrink:0, whiteSpace:"nowrap" }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background=`${C.gold}22`; (e.currentTarget as HTMLElement).style.borderColor=`${C.gold}55`; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background=`${C.gold}12`; (e.currentTarget as HTMLElement).style.borderColor=`${C.gold}30`; }}>
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:16 }}>
      {cards.map(({ value, label, sub, accent, Icon, trend }, i) => {
        ...
        return (
          <div key={label}
            style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 18px", position:"relative", overflow:"hidden", transition:"border-color .2s, transform .2s", cursor:"default", animation:`fadeUp .45s ease ${i*0.06}s both` }}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              ...
              <span style={{ fontSize:11, fontWeight:700, color:tc, fontFamily:FB, display:"flex", alignItems:"center", gap:2, background:`${tc}10`, padding:"3px 7px", borderRadius:20, border:`1px solid ${tc}20` }}>
                <TI size={10} strokeWidth={2.5} />{pos ? "+" : ""}{trend}%
              </span>
            </div>
            <div style={{ fontSize:28, fontWeight:900, color: loading ? C.creamMut : `${accent}DD`, letterSpacing:"-0.5px", lineHeight:1, marginBottom:5, fontFamily:FD, transition:"color .3s" }}>
              {loading ? "—" : fmt(value)}
            </div>
  ...
  const acciones = [
    {
      ...
      Icon: Clock,
      color: C.gold,
      path: "/admin/obras?estado=pendiente",
      gradient: `linear-gradient(135deg, ${C.gold}18, ${C.orange}08)`,
    },
    {
      label: "Artistas",
      sub: "Gestionar artistas",
      Icon: Users,
      color: C.pink,
      path: "/admin/artistas",
      gradient: `linear-gradient(135deg, ${C.pink}18, ${C.purple}08)`,
    },
    {
      label: "Reportes",
      sub: "Ver métricas",
      ...
      color: C.blue,
      path: "/admin/reportes",
      gradient: `linear-gradient(135deg, ${C.blue}18, ${C.purple}08)`,
    },
    {
      label: "Importar",
      sub: "Cargar datos externos",
      Icon: Upload,
      color: C.purple,
      path: "/admin/importar",
      gradient: `linear-gradient(135deg, ${C.purple}18, ${C.blue}08)`,
    },
    {
      label: "Estadísticas",
      sub: "Análisis completo",
      Icon: Activity,
      color: C.green,
        ...
        {acciones.map(({ label, sub, Icon, color, path, gradient }, i) => (
          <button key={label} onClick={() => navigate(path)}
            ...
            onMouseEnter={e => {
              ...
              el.style.boxShadow = `0 12px 28px ${color}18`;
            }}
            onMouseLeave={e => {
              const el = e.currentTarget as HTMLElement;
            ...
            <div>
              <div style={{ fontSize:13.5, fontWeight:700, color:C.cream, marginBottom:3, lineHeight:1.2 }}>{label}</div>
              <div style={{ fontSize:11, color:C.creamMut, lineHeight:1.3 }}>{sub}</div>
            </div>
            <ArrowUpRight size={12} color={color} style={{ position:"absolute", top:12, right:12, opacity:0.5 }} />
          </button>
        ))}
      </div>
    </div>
...
function ChartSection() {
  ...
  const chartTypes = [{ id:"area" as const, Icon:AreaIcon }, { id:"bar" as const, Icon:BarChartIcon }, { id:"line" as const, Icon:LineChartIcon }];
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden", height:"100%", display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"14px 16px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ fontSize:10, color:C.creamMut, fontFamily:FB, marginTop:1 }}>Últimos 4 meses</div>
        </div>
        <div style={{ display:"flex", gap:2, background:"rgba(255,232,200,0.04)", border:`1px solid ${C.border}`, borderRadius:6, padding:2 }}>
          {chartTypes.map(({ id, Icon }) => (
            <button key={id} onClick={() => setChartType(id)}
              style={{ width:26, height:26, borderRadius:4, border:"none", background: chartType===id ? "rgba(255,132,14,0.18)" : "transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ display:"flex", gap:16, padding:"10px 16px 4px" }}>
        {[{c:C.orange,l:"Ventas"},{c:C.blue,l:"Obras"},{c:C.purple,l:"Artistas"}].map(({c,l}) => (
          <span key={l} style={{ display:"flex", alignItems:"center", gap:5, fontSize:10.5, color:C.creamMut, fontFamily:FB }}>
            <span style={{ width:8, height:8, borderRadius:"50%", background:c, display:"inline-block" }} />{l}
          </span>
        ))}
      </div>
      <div style={{ flex:1, padding:"0 12px 10px", minHeight:140 }}>
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px", display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        ...
        <button onClick={() => navigate("/admin/obras")}
          style={{ display:"flex", alignItems:"center", gap:3, background:"transparent", border:`1px solid ${C.border}`, borderRadius:8, padding:"5px 10px", color:C.creamMut, fontSize:11.5, fontWeight:600, cursor:"pointer", fontFamily:FB, transition:"all .15s" }}
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
      {items.map(({ value, label, sub, accent, Icon }, i) => (
        <div key={label}
          style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px", display:"flex", alignItems:"center", gap:16, transition:"border-color .2s, transform .2s", cursor:"default", position:"relative", overflow:"hidden", animation:`fadeUp .5s ease ${0.25+i*0.08}s both` }}
          onMouseEnter={e => { const el=e.currentTarget as HTMLElement; el.style.borderColor=`${accent}28`; el.style.transform="translateY(-2px)"; }}
          onMouseLeave={e => { const el=e.currentTarget as HTMLElement; el.style.borderColor=C.border; el.style.transform="translateY(0)"; }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg, ${accent}, transparent)` }} />
          <div style={{ width:42, height:42, borderRadius:11, flexShrink:0, background:`${accent}12`, border:`1px solid ${accent}22`, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <Icon size={19} color={accent} strokeWidth={1.8} />
          </div>
          <div>
            <div style={{ fontSize:28, fontWeight:900, color: loading ? C.creamMut : C.cream, letterSpacing:"-0.5px", lineHeight:1, marginBottom:4, fontFamily:FD, transition:"color .3s" }}>
          ...
          <ArrowUpRight size={14} color={accent} style={{ marginLeft:"auto", flexShrink:0, opacity:0.5 }} />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <>
      ...
      <main style={{ flex:1, padding:"22px 26px 28px", overflowY:"auto" }}>
        ...
        <AlertaPendientes count={stats?.kpis?.obras_pendientes ?? 0} navigate={navigate} />
