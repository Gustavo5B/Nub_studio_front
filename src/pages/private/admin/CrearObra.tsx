...
const C = {
  ...
  borderHi:"rgba(255,200,150,0.18)", input:"rgba(255,232,200,0.04)",
...
/* ═══ Sidebar ═══ */
function SidebarHeader({ navigate, userName }:{ navigate:(p:string)=>void; userName:string }) {
  return (
    <div style={{ padding:"20px 18px 16px", borderBottom:`1px solid ${C.borderBr}` }}>
      <div role="button" tabIndex={0} onClick={()=>navigate("/")} onKeyDown={e=>{if(e.key==="Enter")navigate("/")}} style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer", marginBottom:16 }}>
        ...
        <div>
          <div style={{ fontSize:14, fontWeight:900, color:C.cream, lineHeight:1.1, fontFamily:FD }}>Galería</div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:12, background:"rgba(255,200,150,0.04)", border:`1px solid ${C.borderBr}` }}>
        <div style={{ width:32, height:32, borderRadius:"50%", flexShrink:0, background:`linear-gradient(135deg,${C.pink},${C.purple})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:800, color:"white", fontFamily:FB }}>{userName?.[0]?.toUpperCase()||"A"}</div>
...
function NavButton({ id, label, icon:Icon, path, active, navigate }:{ id:string; label:string; icon:React.ElementType; path:string; active:string; navigate:(p:string)=>void }) {
  const on = active===id;
  return (
    <button onClick={()=>navigate(path)} style={{ width:"100%", cursor:"pointer", background:on?"rgba(255,132,14,0.10)":"transparent", border:on?"1px solid rgba(255,132,14,0.22)":"1px solid transparent", borderRadius:10, padding:"10px 12px", display:"flex", alignItems:"center", gap:10, transition:"all .15s", position:"relative", fontFamily:FB }}
      ...
      <div style={{ width:32, height:32, borderRadius:9, flexShrink:0, background:on?"rgba(255,132,14,0.15)":"rgba(255,232,200,0.05)", display:"flex", alignItems:"center", justifyContent:"center", border:on?"1px solid rgba(255,132,14,0.25)":"1px solid transparent" }}>
...
function SidebarFooter({ navigate }:{ navigate:(p:string)=>void }) {
  return (
    <div style={{ padding:"12px 10px 18px", borderTop:`1px solid ${C.borderBr}` }}>
      <div style={{ display:"flex", gap:6 }}>
        <button style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:5, padding:"9px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", cursor:"pointer", fontSize:12, color:C.creamMut, fontWeight:600, fontFamily:FB }}><Settings size={13} strokeWidth={1.8}/> Config</button>
        <button onClick={()=>{authService.logout();navigate("/login");}} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:5, padding:"9px", borderRadius:9, border:`1px solid rgba(204,89,173,0.25)`, background:"rgba(204,89,173,0.06)", cursor:"pointer", fontSize:12, color:C.pink, fontWeight:600, fontFamily:FB }}><LogOut size={13} strokeWidth={1.8}/> Salir</button>
      </div>
    </div>
  );
}
...
function Sidebar({ navigate }:{navigate:(p:string)=>void}) {
  ...
  return (
    <div style={{ width:220, minHeight:"100vh", background:C.bgDeep, borderRight:`1px solid ${C.borderBr}`, display:"flex", flexDirection:"column", position:"sticky", top:0, height:"100vh", flexShrink:0, zIndex:40 }}>
      ...
      <SidebarHeader navigate={navigate} userName={userName} />
      <div style={{ flex:1, padding:"12px 10px", display:"flex", flexDirection:"column", gap:2, overflowY:"auto" }}>
        ...
        {NAV.map(n=><NavButton key={n.id} {...n} active="obras" navigate={navigate} />)}
      </div>
...
/* ═══ Topbar ═══ */
function ObraTopbar({ navigate, loading }:{navigate:(p:string)=>void;loading:boolean}) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", height:56, background:C.bgDeep, borderBottom:`1px solid ${C.borderBr}`, position:"sticky", top:0, zIndex:30, fontFamily:FB }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <ChevronRight size={12} color={C.creamMut}/><span style={{ fontSize:13, color:C.creamSub }}>Nueva</span>
      </div>
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={()=>navigate("/admin/obras")} disabled={loading} style={{ padding:"7px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.creamSub, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:FB }}>Cancelar</button>
        <button form="form-crear-obra" type="submit" disabled={loading} style={{ display:"flex", alignItems:"center", gap:7, padding:"7px 18px", borderRadius:9, border:"none", background:loading?`${C.orange}40`:`linear-gradient(135deg,${C.orange},${C.magenta})`, color:"white", fontSize:13, fontWeight:800, cursor:loading?"not-allowed":"pointer", fontFamily:FB, boxShadow:loading?"none":`0 4px 14px ${C.orange}30` }}>
          {loading?<><Loader2 size={14} style={{ animation:"spin 1s linear infinite" }}/> Guardando…</>:<><Save size={14} strokeWidth={2.5}/> Publicar Obra</>}
        </button>
      </div>
    </div>
  );
...
/* ═══ Preview Card ═══ */
function ObraPreviewCard({ form, previewSrc, artist, cat }:{ form:Obra; previewSrc:string; artist?:Artista; cat?:Categoria }) {
  return (
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden", marginBottom:14, position:"relative", animation:"fadeUp .5s ease .05s both" }}>
      ...
      <div style={{ padding:"12px 16px 16px" }}>
        ...
        {artist&&(<div style={{ fontSize:12, color:C.creamSub, marginBottom:10, fontFamily:FB, display:"flex", alignItems:"center", gap:4 }}><Star size={8} color={C.gold} fill={C.gold}/>{artist.nombre_artistico||artist.nombre_completo}</div>)}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7 }}>
          <div style={{ background:`${C.gold}0D`, border:`1px solid ${C.gold}20`, borderRadius:9, padding:"7px 10px" }}>
            ...
            <div style={{ fontSize:13, fontWeight:900, color:form.precio_base?C.gold:C.creamMut, fontFamily:FD }}>{form.precio_base?`$${fmt(Number(form.precio_base))}`:"—"}</div>
          </div>
...
/* ═══ Imagen Card ═══ */
function ImagenUploadArea({ imgFile, imgPreview, fileRef, clearFile, dragOver, setDragOver, onDrop }:{ imgFile:File|null; imgPreview:string; fileRef:React.RefObject<HTMLInputElement>; clearFile:()=>void; dragOver:boolean; setDragOver:(v:boolean)=>void; onDrop:(e:React.DragEvent)=>void }) {
  if (imgFile) {
    return (
  ...
  return (
    <div onClick={()=>fileRef.current?.click()} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={onDrop}
      ...
      <div style={{ textAlign:"center" }}><div style={{ fontSize:12, fontWeight:700, color:dragOver?C.pink:C.creamSub, fontFamily:FB }}>{dragOver?"Suelta aquí":"Arrastra o haz clic"}</div><div style={{ fontSize:10.5, color:C.creamMut, fontFamily:FB }}>JPG, PNG, WEBP · Máx 10 MB</div></div>
    </div>
  );
}

function ImagenUrlInput({ form, onChange, clearFile, focused, loading, fi }:{ form:Obra; onChange:(e:ChangeEvent<HTMLInputElement>)=>void; clearFile:()=>void; focused:string|null; loading:boolean; fi:(n:string)=>{onFocus:()=>void;onBlur:()=>void} }) {
  return (<>
...
function ImagenCard({ form, imgMode, setImgMode, imgFile, imgPreview, fileRef, clearFile, dragOver, setDragOver, onDrop, onChange, uploading, focused, loading, fi }:{
  form:Obra; imgMode:"upload"|"url"; setImgMode:(m:"upload"|"url")=>void; imgFile:File|null; imgPreview:string; fileRef:React.RefObject<HTMLInputElement>; clearFile:()=>void; dragOver:boolean; setDragOver:(v:boolean)=>void; onDrop:(e:React.DragEvent)=>void; onChange:(e:ChangeEvent<HTMLInputElement>)=>void; uploading:boolean; focused:string|null; loading:boolean; fi:(n:string)=>{onFocus:()=>void;onBlur:()=>void};
}) {
  return (
    <Card accent={C.pink} icon={ImageIcon} title="Imagen principal" delay={0.12}>
      ...
      <input ref={fileRef} type="file" accept="image/*" style={{ display:"none" }} onChange={e=>{const f=e.target.files?.[0]; if(f) handleFileFromRef(f);}} />
      {imgMode==="upload" ? <ImagenUploadArea imgFile={imgFile} imgPreview={imgPreview} fileRef={fileRef} clearFile={clearFile} dragOver={dragOver} setDragOver={setDragOver} onDrop={onDrop}/> : <ImagenUrlInput form={form} onChange={onChange} clearFile={clearFile} focused={focused} loading={loading} fi={fi}/>}
      {uploading&&(<div style={{ display:"flex", alignItems:"center", gap:8, marginTop:10, padding:"9px 12px", borderRadius:9, background:`${C.orange}10`, border:`1px solid ${C.orange}28` }}><Loader2 size={13} color={C.orange} style={{ animation:"spin 1s linear infinite" }}/><span style={{ fontSize:12, color:C.orange, fontFamily:FB, fontWeight:600 }}>Subiendo imagen…</span></div>)}
    </Card>
  );
}

// Placeholder for fileRef handler — actual handler is in parent
function handleFileFromRef(_f: File) { /* noop - parent handles via useEffect */ }

/* ═══ Precio Card ═══ */
function PrecioCard({ form, onChange, focused, loading, fi }:{ form:Obra; onChange:(e:ChangeEvent<HTMLInputElement>)=>void; focused:string|null; loading:boolean; fi:(n:string)=>{onFocus:()=>void;onBlur:()=>void} }) {
  return (
...
/* ═══ Resumen Card ═══ */
function ResumenCard({ form, artist, cat }:{ form:Obra; artist?:Artista; cat?:Categoria }) {
  const items = [
  ...
  return (
    <div style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, padding:"16px 18px", position:"relative", overflow:"hidden", animation:"fadeUp .5s ease .16s both" }}>
      ...
      {items.map(({ label, val, color },i)=>(<div key={label} style={{ display:"flex", justifyContent:"space-between", borderBottom:i<items.length-1?`1px solid rgba(255,232,200,0.05)`:"none", paddingBottom:9, marginBottom:9 }}><span style={{ fontSize:11, color:C.creamSub, fontFamily:FB }}>{label}</span><span style={{ fontSize:12.5, fontWeight:700, color:color||C.cream, maxWidth:160, textAlign:"right", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontFamily:label==="Precio"?FD:FB }}>{val}</span></div>))}
    </div>
  );
}

/* ═══ Submit helpers ═══ */
function validateObraForm(form:Obra, imgFile:File|null): string|null {
  if (!form.titulo||!form.descripcion) return "Completa título y descripción";
  if (!form.id_categoria) return "Selecciona una categoría";
  if (!form.id_artista) return "Selecciona un artista";
  if (!form.precio_base||form.precio_base<=0) return "El precio debe ser mayor a 0";
  if (!imgFile&&!form.imagen_principal) return "Agrega una imagen (archivo o URL)";
  return null;
}

function buildObraFormData(form:Obra, imgFile:File|null): FormData {
  const fd = new FormData();
  fd.append("titulo", form.titulo); fd.append("descripcion", form.descripcion);
  fd.append("id_categoria", String(form.id_categoria)); fd.append("id_artista", String(form.id_artista));
  fd.append("precio_base", String(form.precio_base)); fd.append("permite_marco", String(form.permite_marco));
...
export default function CrearObra() {
  ...
  const clearFile = () => {
    ...
    setImgFile(null); setImgPreview(""); if (fileRef.current) fileRef.current.value="";
  };
  ...
  const onSubmit = async (e:FormEvent) => {
    e.preventDefault();
    const error = validateObraForm(form, imgFile);
    if (error) { showToast(error, "warn"); return; }

    setLoading(true);
    if (imgFile) setUploading(true);
    try {
      const fd = buildObraFormData(form, imgFile);
      const res = await fetch(`${API_URL}/api/obras`, { method:"POST", headers:{ Authorization:`Bearer ${authService.getToken()}` }, body:fd });
      setUploading(false);
    ...
    } catch (err) { setUploading(false); showToast(handleNetworkError(err),"err"); }
    finally { setLoading(false); }
  };

  // Handle file input at parent level
  useEffect(() => {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:C.bg, fontFamily:FB, color:C.cream }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        <ObraTopbar navigate={navigate} loading={loading} />
        <main style={{ flex:1, padding:"22px 26px 28px", overflowY:"auto" }}>
          <div style={{ marginBottom:20, animation:"fadeUp .4s ease both" }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:6 }}><Star size={9} color={C.gold} fill={C.gold}/><span style={{ fontSize:10.5, fontWeight:700, color:C.creamMut, textTransform:"uppercase", letterSpacing:"0.12em", fontFamily:FB }}>Catálogo · Nueva pieza</span></div>
            <h1 style={{ fontSize:24, fontWeight:900, margin:0, fontFamily:FD, color:C.cream }}> Agregar{" "}<span style={{ background:`linear-gradient(90deg,${C.orange},${C.gold})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Nueva Obra</span></h1>
          </div>
          ...
          <form id="form-crear-obra" onSubmit={onSubmit}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:14, alignItems:"start" }}>
              <div>
                <Card accent={C.orange} icon={Type} title="Información básica" delay={0.05}>
                  <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                    <div><Lbl req>Título de la obra</Lbl><input name="titulo" value={form.titulo} onChange={onChange} placeholder="Ej: Amanecer en la Huasteca" required disabled={loading} style={IS(focused==="titulo",loading)} {...fi("titulo")}/></div>
                    <div><Lbl req><FileText size={10}/> Descripción</Lbl><textarea name="descripcion" value={form.descripcion} onChange={onChange} placeholder="Describe la obra…" rows={4} required disabled={loading} style={{ ...IS(focused==="desc",loading), resize:"vertical" as const }} {...fi("desc")}/></div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                      ...
                      <div><Lbl><Calendar size={10}/> Año de creación</Lbl><input type="number" name="anio_creacion" value={form.anio_creacion||""} onChange={onChange} min="1900" max={new Date().getFullYear()} disabled={loading} style={IS(focused==="anio",loading)} {...fi("anio")}/></div>
                    </div>
                  </div>
                </Card>
                <Card accent={C.blue} icon={Ruler} title="Dimensiones (cm)" delay={0.08}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
                    {([{name:"dimensiones_alto",label:"Alto",ph:"50"},{name:"dimensiones_ancho",label:"Ancho",ph:"70"},{name:"dimensiones_profundidad",label:"Profundidad",ph:"5"}] as const).map(({name,label,ph})=>(<div key={name}><Lbl>{label}</Lbl><input type="number" name={name} value={(form[name as keyof Obra] as number)||""} onChange={onChange} placeholder={ph} step="0.01" min="0" disabled={loading} style={IS(focused===name,loading)} {...fi(name)}/></div>))}
                  </div>
                  <div style={{ marginTop:10, padding:"9px 12px", borderRadius:9, background:`${C.blue}08`, border:`1px solid ${C.blue}18`, display:"flex", alignItems:"center", gap:7 }}><Ruler size={12} color={C.blue} strokeWidth={1.8}/><span style={{ fontSize:11.5, color:C.creamMut, fontFamily:FB }}>Ingresa las medidas en centímetros.</span></div>
                </Card>
              ...
              <div>
                <ObraPreviewCard form={form} previewSrc={previewSrc} artist={artist} cat={cat}/>
                <PrecioCard form={form} onChange={onChange} focused={focused} loading={loading} fi={fi}/>
                <ImagenCard form={form} imgMode={imgMode} setImgMode={setImgMode} imgFile={imgFile} imgPreview={imgPreview} fileRef={fileRef as React.RefObject<HTMLInputElement>} clearFile={clearFile} dragOver={dragOver} setDragOver={setDragOver} onDrop={onDrop} onChange={onChange} uploading={uploading} focused={focused} loading={loading} fi={fi}/>
                <ResumenCard form={form} artist={artist} cat={cat}/>
              </div>
            </div>
          </form>
        </main>
