                ...
                <Card accent={C.blue} icon={Ruler} title="Dimensiones (cm)" delay={0.1}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
                    {[
                      ...
                      <div key={name}>
                        <Label>{label}</Label>
                        <input type="number" name={name} value={(form[name as keyof Obra] as number) || ""} onChange={onChange} placeholder={ph} step="0.01" min="0" disabled={loading} style={inputStyle(focused === name, loading)} {...fi(name)} />
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ padding:"16px 20px 20px" }}>
                    ...
                    {artist && (
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:12 }}>
                        ...
                        <span style={{ fontSize:12.5, color:C.creamSub, fontFamily:FB }}>{artist.nombre_artistico || artist.nombre_completo}</span>
                      </div>
                    )}
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      ...
                      <div style={{ background:`${C.blue}10`, border:`1px solid ${C.blue}20`, borderRadius:10, padding:"8px 10px" }}>
                        ...
                        <div style={{ fontSize:12.5, fontWeight:700, color:cat ? C.blue : C.creamMut, fontFamily:FB }}>{cat?.nombre || "—"}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div style={{ display:"flex", marginBottom:14, borderRadius:10, overflow:"hidden", border:`1px solid ${C.border}`, background:C.input }}>
                    {(["upload", "url"] as const).map(tab => (
                      <button key={tab} type="button" onClick={() => setImgMode(tab)}
                        style={{ flex:1, padding:"9px", border:"none", cursor:"pointer", fontFamily:FB, fontSize:12.5, fontWeight:imgMode===tab ? 800 : 500, background:imgMode===tab ? `linear-gradient(135deg, ${C.pink}25, ${C.purple}15)` : "transparent", color:imgMode===tab ? C.cream : C.creamMut, borderRight:tab==="upload" ? `1px solid ${C.border}` : "none", transition:"all .15s" }}>
                      ...
                      {imgFile ? (
                        ...
                        <div onClick={() => fileRef.current?.click()}
                          ...
                          <div style={{ textAlign:"center" }}>
                            <div style={{ fontSize:13, fontWeight:700, color:dragOver ? C.pink : C.creamSub, fontFamily:FB }}>{dragOver ? "Suelta aquí" : "Arrastra o haz clic para subir"}</div>
                ...
                <div style={{ background:C.card, borderRadius:18, border:`1px solid ${C.border}`, padding:"18px 20px", backdropFilter:"blur(20px)", position:"relative", overflow:"hidden", animation:"fadeUp .5s ease .2s both" }}>
                  ...
                  {[
                    ...
                    <div key={label} style={{ display:"flex", justifyContent:"space-between", fontSize:12.5, borderBottom:i < arr.length-1 ? `1px solid rgba(255,232,200,0.05)` : "none", paddingBottom:10, marginBottom:10 }}>
                      ...
                      <span style={{ fontWeight:700, color:color || C.cream, maxWidth:160, textAlign:"right", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontFamily:label === "Precio" ? FD : FB }}>{val}</span>
                    </div>
                  ))}
                </div>
              </div>
