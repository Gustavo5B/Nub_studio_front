...
export default function EditarArtista() {
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Validación en tiempo real - ORDEN CORRECTO: XSS primero, luego patrón, luego longitud
    if (name === "nombre_completo") {
      if (value && hasSuspiciousContent(value)) {
        setFieldErrors(p => ({ ...p, nombre_completo: "Contenido no permitido" }));
      } else if (value && !soloLetrasEspacios.test(value)) {
        setFieldErrors(p => ({ ...p, nombre_completo: "Solo letras y espacios — sin números" }));
      } else if (value && value.trim().length < 3) {
        setFieldErrors(p => ({ ...p, nombre_completo: "Mínimo 3 caracteres" }));
      } else {
        setFieldErrors(p => { const n = { ...p }; delete n.nombre_completo; return n; });
      }
    }
    
    if (name === "nombre_artistico") {
      if (value && hasSuspiciousContent(value)) {
        setFieldErrors(p => ({ ...p, nombre_artistico: "Contenido no permitido" }));
      } else if (value && value.trim().length < 2) {
        setFieldErrors(p => ({ ...p, nombre_artistico: "Mínimo 2 caracteres" }));
      } else {
        setFieldErrors(p => { const n = { ...p }; delete n.nombre_artistico; return n; });
      }
    }
    
    if (name === "biografia") {
      if (value && hasSuspiciousContent(value)) {
        setFieldErrors(p => ({ ...p, biografia: "Contenido no permitido" }));
      } else if (value && value.trim().length < 10) {
        setFieldErrors(p => ({ ...p, biografia: "Mínimo 10 caracteres" }));
      } else {
        setFieldErrors(p => { const n = { ...p }; delete n.biografia; return n; });
      }
    }
    
    if (name === "telefono") {
      const digits = value.replace(/[\s\-()]/g, "");
      if (value && !/^\d*$/.test(digits)) {
        setFieldErrors(p => ({ ...p, telefono: "Solo se permiten números" }));
      } else if (value && digits.length > 10) {
        setFieldErrors(p => ({ ...p, telefono: "Máximo 10 dígitos" }));
      } else if (value && digits.length < 10) {
        setFieldErrors(p => ({ ...p, telefono: "Mínimo 10 dígitos" }));
      } else {
        setFieldErrors(p => { const n = { ...p }; delete n.telefono; return n; });
      }
    }

    setForm(p => ({ ...p, [name]: newVal } as typeof form));
  };
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (fotoFile) {
        ...
        const { matricula: formSinMat } = form as typeof form & { matricula: string };
        Object.entries(formSinMat).forEach(([k, v]) => fd.append(k, String(v)));
        ...
        const { matricula: formData } = form as typeof form & { matricula: string };
        res = await fetch(`${API_URL}/api/artistas/${id}`, {
          method: "PUT",
    ...
    <>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", height:56, background:C.bgDeep, borderBottom:`1px solid ${C.borderBr}`, position:"sticky", top:0, zIndex:30, fontFamily:FB }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          ...
          <button form="form-editar-artista" type="submit" disabled={loading}
      ...
      <main style={{ flex:1, padding:"22px 26px 28px", overflowY:"auto" }}>
        <div style={{ marginBottom:20, animation:"fadeUp .4s ease both" }}>
          ...
          <h1 style={{ fontSize:24, fontWeight:900, margin:0, fontFamily:FD, color:C.cream, letterSpacing:"-0.02em" }}>
            Editar{" "}
            <span style={{ background:`linear-gradient(90deg, ${C.pink}, ${C.purple})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{form.nombre_completo || "Artista"}</span>
          </h1>
        </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                    <div>
                      ...
                      <input name="nombre_completo" value={form.nombre_completo} onChange={onChange} required disabled={loading}
                        placeholder="Ej: María López Martínez"
                        style={inputStyle(focused === "nc", loading, !!fieldErrors.nombre_completo)} {...fi("nc")} />
                      <FieldError msg={fieldErrors.nombre_completo} />
                    </div>
                    <div>
                      <Label>Nombre artístico</Label>
                      <input name="nombre_artistico" value={form.nombre_artistico} onChange={onChange} disabled={loading}
                        placeholder="Alias o seudónimo"
                        style={inputStyle(focused === "na", loading, !!fieldErrors.nombre_artistico)} {...fi("na")} />
                      <FieldError msg={fieldErrors.nombre_artistico} />
                    </div>
                  </div>
                  <div>
                    <Label><FileText size={10} /> Biografía</Label>
                    <textarea name="biografia" value={form.biografia} onChange={onChange} rows={4} disabled={loading}
                      placeholder="Cuéntanos sobre el artista, su trayectoria y estilo… (mínimo 10 caracteres)"
                      style={{ ...inputStyle(focused === "bio", loading, !!fieldErrors.biografia), resize:"vertical" as const }} {...fi("bio")} />
              ...
              <Card accent={C.blue} icon={Phone} title="Contacto" delay={0.08}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  <div>
                    ...
                    <input type="email" name="correo" value={form.correo} onChange={onChange} disabled={loading}
                  ...
                  <div>
                    ...
                    <input name="telefono" value={form.telefono} onChange={onChange} disabled={loading}
                      placeholder="7711234567" maxLength={10} inputMode="numeric"
                      style={inputStyle(focused === "tel", loading, !!fieldErrors.telefono)} {...fi("tel")} />
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, background:`${C.purple}0D`, border:`1.5px dashed ${C.purple}30`, minHeight:44 }}>
                      ...
                      <div>
                        <div style={{ fontSize:12, fontWeight:800, color:C.purple, fontFamily:FB, letterSpacing:1 }}>{(form as typeof form & { matricula: string }).matricula || `GAL-${anio}-XXXX`}</div>
