...
/* ═══ Per-field validators (reduces onChange complexity) ═══ */
...
/* ═══ Topbar ═══ */
function EditarArtistaTopbar({ navigate, loading }:{navigate:(p:string)=>void; loading:boolean}) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", height:56, background:C.bgDeep, borderBottom:`1px solid ${C.borderBr}`, position:"sticky", top:0, zIndex:30, fontFamily:FB }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        <button onClick={()=>navigate("/admin/artistas")} style={{ display:"flex", alignItems:"center", gap:6, background:"transparent", border:"none", cursor:"pointer", color:C.creamMut, fontSize:11.5, fontWeight:700, fontFamily:FB, letterSpacing:"0.08em", textTransform:"uppercase" }}
        ...
        <ChevronRight size={12} color={C.creamMut}/><span style={{ fontSize:11.5, fontWeight:700, color:C.orange, letterSpacing:"0.08em", textTransform:"uppercase" }}>Artistas</span>
        <ChevronRight size={12} color={C.creamMut}/><span style={{ fontSize:13, color:C.creamSub }}>Editar</span>
      </div>
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={()=>navigate("/admin/artistas")} disabled={loading} style={{ padding:"7px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.creamSub, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:FB }}>Cancelar</button>
        <button form="form-editar-artista" type="submit" disabled={loading} style={{ display:"flex", alignItems:"center", gap:7, padding:"7px 18px", borderRadius:9, border:"none", background:loading?`${C.orange}40`:`linear-gradient(135deg,${C.orange},${C.magenta})`, color:"white", fontSize:13, fontWeight:800, cursor:loading?"not-allowed":"pointer", fontFamily:FB, boxShadow:loading?"none":`0 4px 14px ${C.orange}30` }}>
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden", marginBottom:14, position:"relative", animation:"fadeUp .5s ease .05s both" }}>
      ...
      <div style={{ height:72, background:`linear-gradient(135deg,${C.pink}28,${C.purple}18,${C.blue}12)`, position:"relative" }}>
        <div style={{ position:"absolute", top:10, right:10, display:"flex", alignItems:"center", gap:5, padding:"3px 9px", borderRadius:100, background:"rgba(10,7,20,0.65)", border:`1px solid ${C.borderHi}` }}>
          ...
          <span style={{ fontSize:9.5, fontWeight:700, color:est?.color||C.creamMut, fontFamily:FB, textTransform:"uppercase", letterSpacing:"0.1em" }}>{est?.label||"Activo"}</span>
        </div>
      </div>
      <div style={{ padding:"0 18px 18px", marginTop:-28 }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:12 }}>
          <div style={{ width:56, height:56, borderRadius:14, background:fotoSrc?"transparent":avatarGrad, border:`3px solid ${C.bg}`, outline:`2px solid ${est?.color||C.pink}40`, overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            {fotoSrc?<img src={fotoSrc} alt="preview" style={{ width:"100%", height:"100%", objectFit:"cover" }} onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>:<span style={{ fontSize:18, fontWeight:900, color:C.cream, fontFamily:FD }}>{initials}</span>}
          </div>
        ...
        <div style={{ fontSize:15, fontWeight:900, color:form.nombre_completo?C.cream:C.creamMut, fontFamily:form.nombre_completo?FD:FB, marginBottom:2 }}>{form.nombre_completo||"Nombre completo"}</div>
        {form.nombre_artistico&&(<div style={{ display:"flex", alignItems:"center", gap:4, marginBottom:8 }}><Star size={8} color={C.gold} fill={C.gold}/><span style={{ fontSize:12, color:C.gold, fontFamily:FB, fontWeight:600 }}>{form.nombre_artistico}</span></div>)}
...
function FotoUploadArea({ fotoFile, fotoPreview, fileRef, clearFoto, dragOver, setDragOver, onDrop }:{
  fotoFile:File|null; fotoPreview:string; fileRef:React.RefObject<HTMLInputElement>; clearFoto:()=>void; dragOver:boolean; setDragOver:(v:boolean)=>void; onDrop:(e:React.DragEvent)=>void;
}) {
  if (fotoFile) {
    return (
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"10px", borderRadius:10, border:`1.5px solid ${C.pink}38`, background:`${C.pink}07`, position:"relative" }}>
        <div style={{ width:52, height:52, borderRadius:10, overflow:"hidden", flexShrink:0, border:`1.5px solid ${C.pink}45` }}><img src={fotoPreview} alt="preview" style={{ width:"100%", height:"100%", objectFit:"cover" }}/></div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}><FileImage size={11} color={C.pink}/><span style={{ fontSize:12, fontWeight:700, color:C.cream, fontFamily:FB, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{fotoFile.name}</span></div>
        ...
        <button type="button" onClick={clearFoto} style={{ position:"absolute", top:7, right:7, width:22, height:22, borderRadius:"50%", background:"rgba(10,7,20,0.80)", border:`1px solid ${C.pink}45`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}><X size={10} color={C.pink}/></button>
      </div>
    );
  }
  return (
...
function FotoUrlInput({ form, onChange, focused, loading, fi, clearFoto }:{form:ArtistaForm; onChange:(e:ChangeEvent<HTMLInputElement>)=>void; focused:string|null; loading:boolean; fi:(n:string)=>{onFocus:()=>void;onBlur:()=>void}; clearFoto:()=>void}) {
  return (<>
  ...
  return (
    <Card accent={C.pink} icon={ImageIcon} title="Foto de perfil" delay={0.1}>
      <div style={{ display:"flex", marginBottom:12, borderRadius:9, overflow:"hidden", border:`1px solid ${C.border}`, background:C.input }}>
        {(["upload","url"] as const).map(tab=>(<button key={tab} type="button" onClick={()=>setFotoMode(tab)} style={{ flex:1, padding:"8px", border:"none", cursor:"pointer", fontFamily:FB, fontSize:12, fontWeight:fotoMode===tab?800:500, background:fotoMode===tab?`${C.pink}18`:"transparent", color:fotoMode===tab?C.cream:C.creamMut, borderRight:tab==="upload"?`1px solid ${C.border}`:"none" }}>{tab==="upload"?<><UploadCloud size={11} style={{ marginRight:4, verticalAlign:"middle" }}/>Subir</>:<><LinkIcon size={11} style={{ marginRight:4, verticalAlign:"middle" }}/>URL</>}</button>))}
      </div>
      <input ref={fileRef} type="file" accept="image/*" style={{ display:"none" }} onChange={e=>{const f=e.target.files?.[0]; if(f){ /* parent handles */ }}} />
      {fotoMode==="upload" ? <FotoUploadArea fotoFile={fotoFile} fotoPreview={fotoPreview} fileRef={fileRef} clearFoto={clearFoto} dragOver={dragOver} setDragOver={setDragOver} onDrop={onDrop}/> : <FotoUrlInput form={form} onChange={onChange} focused={focused} loading={loading} fi={fi} clearFoto={clearFoto}/>}
    </Card>
  );
}

/* ═══ Comision Card ═══ */
function ComisionCard({ form, onChange, focused, loading, fi }:{form:ArtistaForm; onChange:(e:ChangeEvent<HTMLInputElement>)=>void; focused:string|null; loading:boolean; fi:(n:string)=>{onFocus:()=>void;onBlur:()=>void}}) {
  ...
  return (
    <Card accent={C.gold} icon={DollarSign} title="Comisión" delay={0.14}>
      ...
      {Number(form.porcentaje_comision)>0&&(<div style={{ marginTop:10, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"9px 12px", borderRadius:9, background:`${C.gold}0D`, border:`1px solid ${C.gold}22` }}><span style={{ fontSize:12, color:C.creamMut, fontFamily:FB }}>Por venta de $10,000</span><span style={{ fontSize:13.5, fontWeight:900, color:C.gold, fontFamily:FD }}>${new Intl.NumberFormat("es-MX").format(comision)} MXN</span></div>)}
    </Card>
  );
}
    ...
    <Card accent={C.orange} icon={Award} title="Estado de la cuenta" delay={0.16}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
        {ESTADOS.map(({val,label,color})=>{
          ...
          return (<button key={val} type="button" onClick={()=>setForm(p=>({...p,estado:val}))} style={{ padding:"12px 8px", borderRadius:10, border:`1.5px solid ${on?`${color}55`:C.border}`, background:on?`${color}14`:"rgba(255,232,200,0.02)", color:on?color:C.creamSub, fontWeight:on?800:400, fontSize:12, cursor:"pointer", fontFamily:FB, transition:"all .15s", position:"relative" }}>{on&&<div style={{ position:"absolute", top:6, right:6, width:5, height:5, borderRadius:"50%", background:color, boxShadow:`0 0 6px ${color}` }}/>}{label}</button>);
        })}
      </div>
    </Card>
  );
...
export default function EditarArtista() {
  ...
  return (
    <>
      <EditarArtistaTopbar navigate={navigate} loading={loading} />
      <main style={{ flex:1, padding:"22px 26px 28px", overflowY:"auto" }}>
        <div style={{ marginBottom:20, animation:"fadeUp .4s ease both" }}>
          <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:6 }}><Star size={9} color={C.gold} fill={C.gold}/><span style={{ fontSize:10.5, fontWeight:700, color:C.creamMut, textTransform:"uppercase", letterSpacing:"0.12em", fontFamily:FB }}>Comunidad · Edición</span></div>
          <h1 style={{ fontSize:24, fontWeight:900, margin:0, fontFamily:FD, color:C.cream }}>Editar{" "}<span style={{ background:`linear-gradient(90deg,${C.pink},${C.purple})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{form.nombre_completo||"Artista"}</span></h1>
        </div>

        <form id="form-editar-artista" onSubmit={onSubmit}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:14, alignItems:"start" }}>
            <div>
              <Card accent={C.pink} icon={Type} title="Información personal" delay={0.05}>
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                    <div><Label req>Nombre completo</Label><input name="nombre_completo" value={form.nombre_completo} onChange={onChange} required disabled={loading} placeholder="Ej: María López Martínez" style={inputStyle(focused==="nc",loading,!!fieldErrors.nombre_completo)} {...fi("nc")}/><FieldError msg={fieldErrors.nombre_completo}/></div>
                    <div><Label>Nombre artístico</Label><input name="nombre_artistico" value={form.nombre_artistico} onChange={onChange} disabled={loading} placeholder="Alias o seudónimo" style={inputStyle(focused==="na",loading,!!fieldErrors.nombre_artistico)} {...fi("na")}/><FieldError msg={fieldErrors.nombre_artistico}/></div>
                  </div>
                  <div>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}><FieldError msg={fieldErrors.biografia}/><span style={{ fontSize:10.5, color:C.creamMut, fontFamily:FB }}>{form.biografia.length} caracteres</span></div>
                  </div>
                </div>
              </Card>
              <Card accent={C.blue} icon={Phone} title="Contacto" delay={0.08}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  <div><Label><Mail size={10}/> Correo electrónico</Label><input type="email" name="correo" value={form.correo} onChange={onChange} disabled={loading} placeholder="artista@correo.com" style={inputStyle(focused==="correo",loading)} {...fi("correo")}/></div>
                  <div><Label><Phone size={10}/> Teléfono <span style={{ color:C.creamMut, fontWeight:400, textTransform:"none" }}>(10 dígitos)</span></Label><input name="telefono" value={form.telefono} onChange={onChange} disabled={loading} placeholder="7711234567" maxLength={10} inputMode="numeric" style={inputStyle(focused==="tel",loading,!!fieldErrors.telefono)} {...fi("tel")}/><FieldError msg={fieldErrors.telefono}/></div>
                </div>
              </Card>
              <Card accent={C.purple} icon={Tag} title="Categoría y matrícula" delay={0.12}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  <div><Label><Palette size={10}/> Disciplina principal</Label><select name="id_categoria_principal" value={form.id_categoria_principal} onChange={onChange} disabled={loading} style={inputStyle(focused==="cat",loading)} {...fi("cat")}><option value="">Sin categoría</option>{categorias.map(c=><option key={c.id_categoria} value={c.id_categoria}>{c.nombre}</option>)}</select></div>
                  <div><Label><Hash size={10}/> Matrícula</Label><div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, background:`${C.purple}0D`, border:`1.5px dashed ${C.purple}30`, minHeight:44 }}><div style={{ width:7, height:7, borderRadius:"50%", background:C.purple, flexShrink:0, boxShadow:`0 0 8px ${C.purple}80` }}/><div><div style={{ fontSize:12, fontWeight:800, color:C.purple, fontFamily:FB, letterSpacing:1 }}>{form.matricula||`GAL-${anio}-XXXX`}</div><div style={{ fontSize:10, color:C.creamMut, fontFamily:FB, marginTop:1 }}>No editable</div></div></div></div>
                </div>
              </Card>
