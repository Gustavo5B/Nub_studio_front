...
const C = {
  ...
  green: "#3DDB85", red: "#FF4D6A", bg: "#0d0b14", card: "rgba(255,255,255,0.025)",
};
...
const css = `
  ...
  @keyframes pulse    { 0%,100%{box-shadow:0 0 0 0 rgba(255,132,14,0.4)} 50%{box-shadow:0 0 0 8px rgba(255,132,14,0)} }

  .no-page{display:flex;min-height:100vh;background:${C.bg};font-family:'DM Sans',sans-serif;color:${C.text}}

  /* sidebar */
  .no-sidebar{width:230px;min-height:100vh;background:rgba(255,255,255,0.018);border-right:1px solid rgba(255,255,255,0.06);padding:28px 18px;display:flex;flex-direction:column;gap:22px;position:sticky;top:0;flex-shrink:0}
  .no-brand{font-size:1.4rem;font-weight:800;letter-spacing:.04em;font-family:'Playfair Display',serif}
  .no-brand-nu{color:${C.text}}.no-brand-b{color:${C.orange}}
  .no-artist-card{display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:11px 13px}
  .no-avatar{width:38px;height:38px;background:linear-gradient(135deg,#FF840E,#CC59AD);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:.95rem;flex-shrink:0}
  .no-artist-name{font-size:.82rem;font-weight:600;margin:0;color:${C.text}}
  .no-artist-badge{font-size:.65rem;background:rgba(61,219,133,0.15);color:${C.green};padding:2px 8px;border-radius:20px;margin-top:2px;display:inline-block}
  .no-nav-label{font-size:.62rem;letter-spacing:.12em;color:rgba(245,240,255,0.25);margin:0 0 6px;font-weight:700}
  .no-nav-links{display:flex;flex-direction:column;gap:3px;flex:1}
  .no-nav-btn{background:none;border:none;color:rgba(245,240,255,0.5);font-size:.86rem;font-family:'DM Sans',sans-serif;text-align:left;padding:9px 13px;border-radius:10px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:all .2s}
  .no-nav-btn:hover{background:rgba(255,255,255,0.05);color:${C.text}}
  .no-nav-btn.active{background:rgba(255,132,14,0.12);color:${C.orange};font-weight:700}
  .no-upload-btn{background:linear-gradient(135deg,#FF840E,#CC59AD);color:#fff;border:none;padding:11px 14px;border-radius:12px;font-weight:700;font-size:.84rem;cursor:pointer;font-family:'DM Sans',sans-serif;transition:opacity .2s}
  .no-upload-btn:hover{opacity:.85}
  .no-logout{background:none;border:none;color:rgba(245,240,255,0.28);font-size:.8rem;cursor:pointer;font-family:'DM Sans',sans-serif;text-align:left;padding:6px 0;transition:color .2s}
  .no-logout:hover{color:${C.red}}

  /* main */
  .no-main{flex:1;padding:40px 48px 60px;max-width:820px;animation:fadeUp .4s ease both}
  ...
  .no-section-icon{font-size:1rem}
  ...
  .no-drop-icon{font-size:2.5rem;opacity:.6}
  ...
  /* success */
  .no-success{min-height:100vh;background:${C.bg};display:flex;align-items:center;justify-content:center;font-family:'DM Sans',sans-serif}
  .no-success-inner{text-align:center;display:flex;flex-direction:column;align-items:center;gap:16px;animation:scaleIn .4s ease}
...
export default function NuevaObra() {
  ...
  const removeImage = () => { setPreview(null); setImageFile(null); if (fileRef.current) fileRef.current.value = ""; };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Perfil incompleto — mostrar banner con campos faltantes
      ...
      <div className="no-page">
        ...
        <aside className="no-sidebar">
          <div className="no-brand">
            <span className="no-brand-nu">NU</span>
            <span className="no-brand-b">★B</span>
          </div>
          <div className="no-artist-card">
            <div className="no-avatar">{authService.getUserName()?.charAt(0).toUpperCase()}</div>
            <div>
              <p className="no-artist-name">{authService.getUserName()}</p>
              <span className="no-artist-badge">Artista activo</span>
            </div>
          </div>
          <nav className="no-nav-links">
            <p className="no-nav-label">NAVEGACIÓN</p>
            <button className="no-nav-btn" onClick={() => navigate("/artista/dashboard")}>⊞ Overview</button>
            <button className="no-nav-btn active" onClick={() => navigate("/artista/mis-obras")}>🖼 Mis obras</button>
            <button className="no-nav-btn" onClick={() => navigate("/artista/perfil")}>👤 Mi perfil</button>
          </nav>
          <button className="no-upload-btn" onClick={() => navigate("/artista/nueva-obra")}>+ Subir nueva obra</button>
          <button className="no-logout" onClick={() => { authService.logout(); navigate("/login"); }}>↪ Cerrar sesión</button>
        </aside>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 1 && (
              <>
                ...
                <div className="no-section">
                  <div className="no-section-title"><span className="no-section-icon">🖼</span> Imagen de la obra</div>
                  {!preview ? (
                    <div className={`no-drop${dragOver ? " over" : ""}`}
                      ...
                      <div className="no-drop-icon">📤</div>
                      <p>Arrastra tu imagen o <span>haz clic para seleccionar</span></p>
                      <small>PNG, JPG, WEBP — Máx 10 MB</small>
                      <input ref={fileRef} type="file" accept="image/*" hidden onChange={e => { const f = e.target.files?.[0]; if (f) processImage(f); }} />
                    </div>
                  ) : (
                ...
                {/* Info básica */}
                <div className="no-section">
                  <div className="no-section-title"><span className="no-section-icon">📝</span> Información básica</div>
                ...
                {etiquetas.length > 0 && (
                  <div className="no-section">
                    <div className="no-section-title"><span className="no-section-icon">🏷</span> Etiquetas</div>
            ...
            {step === 2 && (
              <>
                {/* Dimensiones */}
                <div className="no-section">
                  <div className="no-section-title"><span className="no-section-icon">📐</span> Dimensiones (cm)</div>
                  <div className="no-grid-3">
                    {[["dimensiones_alto","Alto"],["dimensiones_ancho","Ancho"],["dimensiones_profundidad","Profundidad"]].map(([name,label]) => (
                      <div key={name} className="no-field">
                        <label className="no-field-label">{label}</label>
                        <input className="no-input" type="number" name={name} value={(form as Record<string,string>)[name]} onChange={handleChange} placeholder="0" min={0} step="0.1" />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Precio */}
                <div className="no-section">
                  <div className="no-section-title"><span className="no-section-icon">💰</span> Precio</div>
                  <div className="no-field">
                ...
                {/* Extras */}
                <div className="no-section">
                  <div className="no-section-title"><span className="no-section-icon">🎨</span> Extras</div>
                ...
                <div className="no-section no-summary">
                  <div className="no-section-title"><span className="no-section-icon">✦</span> Resumen</div>
